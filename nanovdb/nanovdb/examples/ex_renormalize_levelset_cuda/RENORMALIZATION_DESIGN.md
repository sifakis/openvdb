...
The `lam/` subtree is a verbatim reference import (see `lam/PROVENANCE.md`).
...
* **§5** -- what NanoVDB already provides versus the delta LAM carries.  Start
  here to size the work.
* **§8** -- proposed order of work.
* **§9** -- `file:line` map into the imported reference code.

Landed on this branch so far: the reference import (`lam/`), this document, and
...
The starting point is the CUDA implementation in the LAM / Sem3D benchmark
(imported under `lam/`), which today lives outside NanoVDB and shadows one of
...
concern (`lam/include/LevelSetPropagate.h`) and is **out of scope** here, even
...
gradient magnitude.  The discretization inherited from OpenVDB and LAM is:
...
* LAM/CUDA: `lam/src/Benchmark.cu:347-355`

### Relationship to OpenVDB

`lam/include/LevelSetTrackerNew.h` is a fork of
...
sidecar**, not a `NanoGrid<float>`.  This matches the LAM implementation and is
...
* `BuildT` (topology encoding, fixed at `ValueOnIndex`) and `ValueT` (sidecar
  element type) become **independent** template parameters.  The LAM code
  currently uses `WenoStencil<nanovdb::FloatGrid>` purely as a carrier for
  `SIZE` and the `WenoPt<>` index map (`lam/src/Benchmark.cu:335`), which is the
...
Instead we depend on the **sign-extrapolation heuristic** already used by LAM: a
...
1. **Stencil gather** (`lam/src/Benchmark.cu:328-345`): a missing neighbor takes
...
2. **Dilation seeding** (`lam/src/Benchmark.cu:108-116`): a newly activated voxel
...
LAM allocates it fresh inside every call (`lam/src/Benchmark.cu:450-451`): a full
...
Tracing the RK2 ping-pong (`lam/src/Benchmark.cu:370-429`): `euler01` reads phi
...
LAM hardcodes the default stream and `UnifiedBuffer` (§7).  The promoted code
...
(`lam/src/Benchmark.cu:286-327`).  It has already been solved once, on this
...
`NVIDIA_Sem3D @ f8ab0ca`.  Also undecided: unit test in `nanovdb/unittest`
versus example-only.

### B6. Wiring the example into CMake

The imported `lam/` tree is deliberately unregistered (`lam/PROVENANCE.md`).
...
The experiment itself: build the LAM harness with NanoVDB's `WENO5` instantiated
...
Everything `lam/src/Benchmark.cu` includes is satisfied by ASWF `master`:
...
### The actual delta LAM carries

1. **`lam/include/Stencils.h`** — 30 diff lines against `nanovdb/math/Stencils.h`:
   two **static, array-based** overloads,
   `WenoStencil::normSqGrad(const ValueType* v, invDx2, dx2, iso)`
   (`lam/include/Stencils.h:665`) and
   `WenoStencil::gradient(const ValueType* v, inv2Dx)`
   (`lam/include/Stencils.h:710`).  They exist because a GPU kernel gathers the
   stencil into registers itself and cannot use the stencil object's accessor.
   This is the entire reason LAM shadows the upstream header (Sem3D commit
...
2. **The 19-point gather** (`lam/src/Benchmark.cu:286-327`) — the `leafPtrs[3][3]`
...
3. **The narrow-band boundary rule** (`lam/src/Benchmark.cu:328-345`), per §3.2/§3.3.
4. **The Euler step** (`lam/src/Benchmark.cu:347-355`) — already line-for-line
...
2. **OpenVDB cross-check** — the existing harness.  The tandem call sites were
   ...
   `NVIDIA_Sem3D @ f8ab0ca:Benchmark/include/LevelSetTrackerNew.h:340-372`, which
   interleaves both stacks and calls `compareOpenVDBDataToNanoVDBSidecar` after
   each stage.

### Resolved: the scheme LAM actually uses

`lam/src/main.cpp:113-117` selects, for both the propagator and the tracker:
...
* NanoVDB's `WENO5` defaults to `scale2 = 1.0` and carries the comment
  ...
  LAM's static overload is called as `normSqGrad(stencil, 1.f, 1.f)`
  (`lam/src/Benchmark.cu:358`) -- so `eps = 1e-6`.
...
`1e-8` is a dx-independent magic number -- but **LAM's `dx2 = 1.f` gets neither**.
...
Both hypotheses are cheaply testable in isolation (build the LAM harness with
...
Inherited from LAM and not to be carried into NanoVDB as-is:

* **Hardcoded default stream** — every launch passes `cudaStream_t(0)` and the
  driver ends in `cudaStreamSynchronize(0)` (`lam/src/Benchmark.cu:459`).
...
* **Hardcoded `UnifiedBuffer`**, including the RK temporary that the CUDA path
  ...
  `lam/src/Benchmark.cu:441-448`.  `cuda::Buffer<T>` landed upstream in `0ab0a81e7`.
...
* **CPU scaffolding inside the CUDA path.**  Since only CUDA is in scope, the
  ...
  `lam/src/Benchmark.cpp` (entirely `ExecutionPolicy::CPU`) has no counterpart in
  ...
  branch that can call the *host* VBM builder (`lam/src/Benchmark.cu:144`,
  `:534`) -- a nominally-CUDA entry point with a runtime escape into host code.
* **Misnamed axis variables** in the dilate seeder (`lam/src/Benchmark.cu:110-115`):
...
* **`gridHandle.reset()` / `phiBuffer.clear()` before move-assign**
  (`lam/src/Benchmark.cu:164-166`), worked around with
...
1. **Upstream the two static `WenoStencil` overloads** into
   `nanovdb/math/Stencils.h` (§5) and drop LAM's shadowed header.  Small,
...
| What | Where |
|---|---|
| CUDA renormalization functor | `lam/src/Benchmark.cu:245` |
| — VBM block decode | `lam/src/Benchmark.cu:283` |
| — 19-point stencil gather | `lam/src/Benchmark.cu:286-327` |
| — sign-extrapolation cascade | `lam/src/Benchmark.cu:328-345` |
| — Euler step | `lam/src/Benchmark.cu:347-355` |
| RK2 driver | `lam/src/Benchmark.cu:436` |
| CUDA dilation + sidecar seeding | `lam/src/Benchmark.cu:80`, `:108-116` |
| CUDA narrow-band prune | `lam/src/Benchmark.cu:474` |
| Static WENO overloads (the delta) | `lam/include/Stencils.h:665`, `:710` |
| Host `ExecutionPolicy::CPU` paths | `lam/src/Benchmark.cpp` |
| OpenVDB/NanoVDB bridge + comparisons | `lam/src/BenchmarkIO.cpp` |
| OpenVDB reference tracker (forked) | `lam/include/LevelSetTrackerNew.h` |
...
| Out of scope: propagation | `lam/include/LevelSetPropagate.h`, `lam/include/VelocityExtension.h` |
