...
`lam/` subtree is a verbatim reference import (see `lam/PROVENANCE.md`).
...
* **How much of OpenVDB's feature matrix?**  Upstream has 5 spatial x 3 temporal
  schemes, masked `normalize(&mask)`, `TrimMode`, `dilate`/`erode`/`resize`.  LAM
...
* **Host path in scope?**  Precedent in this tree (`DilateGrid.h` /
  `DilateGrid.cuh`) says host+device.  LAM's `ExecutionPolicy` templating was
  built for exactly that, and `normalizeLevelSet<ExecutionPolicy::CPU>` exists
  (`lam/src/Benchmark.cpp:224`) — but the host WENO kernel itself is the
  unfinished part (`lam/CPU_PORT_PLAN.md` Phase 2).
